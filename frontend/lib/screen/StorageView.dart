...
Future<void> fetchupdate(String id, String title) async {
  Map<String, String> requestBody = {
    'name': title,
  };
  final response = await http.post(
    Uri.parse('$serverUrl/routes/$id/update'),
    headers: {
...
class _StorageViewState extends State<StorageView> {
  ...
  Future<void> _updateTitle() async {
    try {
      await fetchupdate(widget.id, _titleController.text);
      setState(() {
        // 데이터를 다시 가져와서 UI를 새로 고침합니다.
        futureStorageData = fetchStorageData(widget.id);
      });
    } catch (e) {
      print('업데이트 오류: $e');
